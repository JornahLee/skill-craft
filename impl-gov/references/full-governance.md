...
每次完整治理必须作为一个独立实施项组织，所有阶段文档归入同一文件夹：
...
- `plan.md` 保存总体实施计划、阶段总表、当前位置与最终结论。
- `stages/` 中每个阶段使用一个独立契约文档，文件名前缀按阶段顺序固定为两位数字；阶段调整时同步更新 `plan.md`。
- 可在实施项文件夹内增加直接服务于本次治理的证据或决策文档，但不得把不同实施项混入同一文件夹。
- 完整治理的所有阶段通过用户关且总体结论补全后，将整个实施项文件夹从 `active/` 移动到 `archive/`。不得只移动 `plan.md`，也不得把阶段文档平铺到 `archive/`。
- 实施明确终止时，在 `plan.md` 记录终止原因、已完成阶段和遗留影响，再将整个文件夹归档。
- 归档后更新 `archive/record.md`，按归档时间倒序增加一行。一个完整实施只记录一行并链接到其 `plan.md`，不为各阶段分别创建时间线记录。
- 归档操作必须保持文档内相对链接有效；移动后检查 `plan.md`、阶段契约和 `record.md` 的链接。
...
| 人工验收场景 | 包含前置条件、逐步操作、每步预期结果、语义判断和用户反馈方式；必须能直接交付用户执行 |
...
工程关通过后，Agent 必须在对话中主动向用户提供本阶段可直接执行的验收步骤，不得只说“请验收”、只给验收目标或要求用户自行查阅阶段契约。验收步骤至少包含：

1. 验收前置条件：运行入口、环境状态、账号或权限，以及需要使用的固定验收数据。
2. 逐步操作：按执行顺序给出具体动作；需要输入时提供确切输入值。
3. 每步预期结果：说明界面、数据、状态或外部行为应出现什么变化。
4. 语义判断点：明确哪些现象需要用户依据真实业务认知判断，而不是让用户重复工程测试。
5. 反馈方式：请用户明确回复“通过”，或指出发生偏差的步骤、实际结果与正确业务含义。

Agent 应先完成自己能够完成的环境准备和可复现性检查，再把需要用户亲自判断的部分交给用户；不得把环境搭建、自动化检查或故障排查伪装成用户验收。若验收有多个场景，按场景分组并保持最小充分，不要求用户探索未说明的路径。

用户尚未实际执行验收并明确反馈结论时，将阶段状态记为“等待用户验收”。以下内容均不构成用户关通过：Agent 自行推断、自动化测试通过、截图看似正确、用户仅确认收到步骤，或用户没有回复。Agent 不得代替用户填写通过结论。

## 验证粒度

### 迭代验证

每个可验证修改批次只运行覆盖当前影响范围的最小充分检查：
...
出现共享影响、数据或安全变化、构建或测试配置变化、跨领域累计修改、影响无法界定、相邻回归、准备合并发布，或者用户与仓库规范明确要求时，扩大验证范围。影响范围暂不清楚时，先检查调用关系、测试映射和项目结构；确实无法界定时才运行全量验证，并记录原因。
...
工程关通过但尚未取得用户结论时，必须向用户交付可直接执行的验收步骤，并将阶段标记为“等待用户验收”；不得以工作已实现或工程验证已通过为由结束完整治理。

所有阶段通过用户关后，还必须补全总体结论、归档完整实施文件夹并更新 `archive/record.md`，完整治理才算完成。
